...
public class Pawn implements Move {
    ...
    public Set<Coordinates> getAllowedMoves(Game game, Coordinates position) {
        ...
        if (color.equals(Color.White)){
            ...
            if (!game.isEmpty(position.getLetterNumber() + 1, position.getNumber() + 1)
                    && game.getColor(position.getLetterNumber() + 1,
                    position.getNumber() + 1).equals(Color.Black)){
        ...
        if (color.equals(Color.Black)){
            ...
            if (!game.isEmpty(position.getLetterNumber() + 1, position.getNumber() - 1)
                    && game.getColor(position.getLetterNumber() + 1,
                    position.getNumber() - 1).equals(Color.White)){
                result.add(new Coordinates(position.getLetterNumber() + 1, position.getNumber() - 1));
            }
            if (!game.isEmpty(position.getLetterNumber() - 1, position.getNumber() - 1)
                    && game.getColor(position.getLetterNumber() - 1,
